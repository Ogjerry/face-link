...
__global__ void adam_update_kernel(
    ...
    for (int i = 0; i < n_float2_per_entry; ++i) {
        ...
        // +++ START NEW CLIPPING LOGIC +++
        // Clip each component of the gradient vector
        grad.x = fmaxf(-grad_clip_val, fminf(grad_clip_val, grad.x));
        grad.y = fmaxf(-grad_clip_val, fminf(grad_clip_val, grad.y));
        // +++ END NEW CLIPPING LOGIC +++
...
void HashTable::adam_update(
    ...
    if (tv_loss_weight > 0.0f && grid != nullptr){
        ...
        dim3 block_dim_tv(256);
        dim3 grid_dim_tv(
            (this->T_per_level * (F_val / 2) + block_dim_tv.x - 1) / block_dim_tv.x,
            this->L
        );
        calculate_tv_grad_kernel<<<grid_dim_tv, block_dim_tv, 0, stream>>>(
