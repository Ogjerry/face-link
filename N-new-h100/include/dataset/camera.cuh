...
struct CameraParameters {
    ...
    __host__ __device__ void generate_ray(
        ...
        // 3. The ray origin is the camera's position (translation part of c2w)
        out_origin.x = c2w.m[3];
        out_origin.y = c2w.m[7];
        out_origin.z = c2w.m[11];
        
        // 4. Normalize the final world-space direction vector
        float norm_inv = rsqrtf(dot(dir_w, dir_w));
        out_direction = dir_w * norm_inv;
        // if (norm > 1e-8f) {
        // } else {
