...
__global__ void fused_backward_fully_connected_kernel(
    ...
    const __half* __restrict__ input,
    const __half* __restrict__ weights,
    const __half* __restrict__ activations, // The output of the forward layer (before activation for ReLU)
...
private:
    ...
    CudaManagedBuffer<__half> m_params_buffer;

    // --- GRADIENT BUFFERS (FP32) ---
    CudaManagedBuffer<float>  m_grads_buffer;

    // --- ADAM OPTIMIZER STATE BUFFERS (FP32) ---
    CudaManagedBuffer<float>  m_optimizer_m_buffer; // For Adam 'm' state
    CudaManagedBuffer<float>  m_optimizer_v_buffer; // For Adam 'v' state
...
public:
    ...
    __half *p_density_w1, *p_density_b1;
    __half *p_density_w2, *p_density_b2;
    __half *p_color_w1,   *p_color_b1;
    __half *p_color_w2,   *p_color_b2;
    __half *p_color_w3,   *p_color_b3;

    // Gradient Pointers
    float *g_density_w1, *g_density_b1;
    float *g_density_w2, *g_density_b2;
    float *g_color_w1,   *g_color_b1;
    float *g_color_w2,   *g_color_b2;
    float *g_color_w3,   *g_color_b3;

    // Optimizer Pointers
    // 1st moment (m)
    float* m_dw1_m, *m_db1_m;
    float* m_dw2_m, *m_db2_m;
    float* m_cw1_m, *m_cb1_m;
    float* m_cw2_m, *m_cb2_m;
    float* m_cw3_m, *m_cb3_m;
    // 2nd moment (v)
    float* m_dw1_v, *m_db1_v;
    float* m_dw2_v, *m_db2_v;
    float* m_cw1_v, *m_cb1_v;
    float* m_cw2_v, *m_cb2_v;
    float* m_cw3_v, *m_cb3_v;


    // --- MULTI-GPU: Add accessors for each GRADIENT buffer ---
    float* all_gradients_ptr() { return m_grads_buffer.get(); }
    size_t all_gradients_size_in_elements() { return m_grads_buffer.size(); }
    size_t all_gradients_size_in_bytes() { return m_grads_buffer.nbytes(); }
    ...
    */
    ...
    __host__ __device__ const __half* density_weights1() const { return p_density_w1; }
    __host__ __device__ const __half* density_biases1()  const { return p_density_b1; }
    __host__ __device__ const __half* density_weights2() const { return p_density_w2; }
    __host__ __device__ const __half* density_biases2()  const { return p_density_b2; }
    __host__ __device__ const __half* color_weights1()   const { return p_color_w1; }
    __host__ __device__ const __half* color_biases1()    const { return p_color_b1; }
    __host__ __device__ const __half* color_weights2()   const { return p_color_w2; }
    __host__ __device__ const __half* color_biases2()    const { return p_color_b2; }
    __host__ __device__ const __half* color_weights3()   const { return p_color_w3; }
    __host__ __device__ const __half* color_biases3()    const { return p_color_b3; }
...
public:
    ...
    void forward(
     ...
     void backward(
        ...
        float* d_grad_hash_features      // Output: the gradient w.r.t. the hash features
    );
};
